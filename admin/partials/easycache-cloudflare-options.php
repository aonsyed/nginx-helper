...
?>
	...
	<div class="ec-cf-settings">
		<form id="post_form" method="post" action="#" name="easycache_cf_settings_form" class="clearfix">
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Cloudflare Settings', 'gridpane-nginx-helper' ); ?></h3>
				<div class="inside">
					<table class="form-table rtnginx-table">
						<tbody>
						<tr valign="top">
							<th scope="row">
								<h4><?php esc_html_e( 'API Token', 'gridpane-nginx-helper' ) ?></h4>
							</th>
							<td>
								<fieldset>
									<legend class="screen-reader-text">
										<?php esc_html_e( 'Cloudflare API Token', 'gridpane-nginx-helper' ) ?>
									</legend>
								</fieldset>
								...
								<p><?php
									if ( $ec_site_settings['api_token_enabled_by_constant'] ) {
										esc_html_e( 'Field enabled by constant.', 'gridpane-nginx-helper' );
									} else {
										esc_html_e( 'Required permissions: Zone.Cache Purge, Zone.Cache Rules, Account.Account Rulesets.Edit,  Account.Account Filter Lists.Edit', 'gridpane-nginx-helper' );
									}
									?>
								</p>
							</td>
						...
						<tr valign="top">
							<th scope="row">
								<h4><?php esc_html_e( 'Zone ID', 'gridpane-nginx-helper' ) ?></h4>
							</th>
							<td>
								<fieldset>
									<legend class="screen-reader-text">
										<?php esc_html_e( 'Cloudflare Zone ID', 'gridpane-nginx-helper' ) ?>
									</legend>
								</fieldset>
						...
						<tr valign="top">
							<th scope="row">
								<h4><?php esc_html_e( 'Default Cache TTL (seconds)', 'gridpane-nginx-helper' ) ?></h4>
							</th>
							<td>
								<fieldset>
									<legend class="screen-reader-text">
										<?php esc_html_e( 'Cloudflare Default Cache TTL', 'gridpane-nginx-helper' ) ?>
									</legend>
								</fieldset>
								<input name="default_cache_ttl" id="default_cache_ttl" type="number"
									   value="<?php echo esc_attr( $ec_site_settings['default_cache_ttl'] ) ?>"/>
								<p class="description"> <?php esc_html_e( 'Default: 604800 seconds (1 week).', 'gridpane-nginx-helper' ); ?> </p>
							</td>
						</tr>
						</tbody>
					</table>
			...
			<?php
			submit_button( __( 'Save Changes', 'gridpane-nginx-helper' ), 'primary large', 'easycache_cf_settings_save', true );
			?>
		</form>
	</div>
...
if ( $ec_site_settings['is_enabled'] ) {
	...
	<form name="easycache_add_cache_rule" method="POST" >
		<?php
			...
			submit_button( __( 'Setup Cache Rules', 'gridpane-nginx-helper' ), 'secondary large', 'easycache_add_cache_rule_save', false );
		?>
